...
<tr class="{{ data_get($theme, 'table.trBodyClass'). ' '.data_get($theme, 'table.trBodyClassTotalColumns') }}"
    style="{{ data_get($theme, 'table.trBodyStyle'). ' '.data_get($theme, 'table.trBodyStyleTotalColumns') }}; border:1px solid #b0b0b0; border-collapse: collapse">
    @if(data_get($setUp, 'detail.showCollapseIcon'))
        <td class="{{ data_get($theme, 'table.tdBodyClass') }}" style="{{ data_get($theme, 'table.tdBodyStyle') }};border:1px solid #b0b0b0; border-collapse: collapse"></td>
    @endif
    @if($checkbox)
        <td class="{{ data_get($theme, 'table.tdBodyClass') }}" style="{{ data_get($theme, 'table.tdBodyStyle') }};border:1px solid #b0b0b0; border-collapse: collapse"></td>
    @endif
    ...
    @if(isset($actions) && count($actions))
        <th class="{{ data_get($theme, 'table.thClass') .' '. $column->headerClass }}" scope="col"
            style="{{ data_get($theme, 'table.thStyle') }};border:1px solid #b0b0b0; border-collapse: collapse" colspan="{{count($actions)}}">
        </th>
    @endif
</tr>
