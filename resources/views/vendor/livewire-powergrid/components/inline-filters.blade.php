...
@if (config('livewire-powergrid.filter') === 'inline')
    <tr
        class="{{ $trClasses }}"
        style="{{ data_get($theme, 'table.trStyle') }} {{ data_get($theme, 'table.trFiltersStyle') }};border:1px solid #b0b0b0; border-collapse: collapse"
    >

        @if (data_get($setUp, 'detail.showCollapseIcon'))
            <td
                class="{{ $tdClasses }}"
                style="{{ $tdStyles }}; border:1px solid #b0b0b0; border-collapse: collapse"
            ></td>
        @endif
        @if ($checkbox)
            <td
        ...
        @foreach ($this->visibleColumns as $column)
            ...
            <td
                ...
                style="{{ data_get($column, 'hidden') === true ? 'display:none' : '' }}; {{ data_get($theme, 'table.tdBodyStyle') }};border:1px solid #b0b0b0; border-collapse: collapse"
            >
